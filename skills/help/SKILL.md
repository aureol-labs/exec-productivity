...
|---|---|---|
| No brief this morning | the last `runs` with `task: morning`; the scheduled tasks list; `connections/current` | A run failed: say what failed in one line, re-run the brief skill now, hand the link. No task: recreate it from `../install/references/task-morning.md` (install step 8, that step only). A connection failed: one connector card. |
